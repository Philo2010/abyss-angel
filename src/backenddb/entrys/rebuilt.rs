...
use sea_orm::{ActiveValue::{NotSet, Set}, ExprTrait, FromQueryResult, QuerySelect, entity::prelude::*, sea_query::Alias};
use serde::{Deserialize, Serialize};
use crate::backenddb::{frontrunnner::{average_field, consensus_field}, game::{self, AvgReturn, FrontRunnerGame, FrontRunnerReturn, GamesAvgSpecific, GamesEditSpecific, GamesFullSpecific, DefenceTarget, GamesInserts, GamesInsertsSpecific, HeaderInsert, InsertReturn, Scores, TeamGameUnMergedData, YearOp}};
use std::hash::Hash;
...
impl YearOp for Functions {
    ...
    fn frontrunner_op(
        ...
        // Generic header consensus (from parallel vecs)
        let defence_main = match consensus_field(&games.defence_mains, &mut crazy, |v: &bool| *v, "defence_main") {
            Some(a) => a,
            None => {
        ...
        let dead = match consensus_field(&games.deads, &mut crazy, |v: &bool| *v, "dead") {
            Some(a) => a,
            None => {
                crazy.extend(0..games.games.len());
                return Ok(FrontRunnerReturn { crazy: crazy.into_iter().collect(), avg: None });
        ...
        let dnf = match consensus_field(&games.dnfs, &mut crazy, |v: &bool| *v, "dnf") {
            Some(a) => a,
            None => {
                crazy.extend(0..games.games.len());
                return Ok(FrontRunnerReturn { crazy: crazy.into_iter().collect(), avg: None });
        ...
        let auto_time = average_field(&games.auto_times, &mut crazy, |v: &f32| *v, "auto_time")?;
    ...
    async fn edit(&self, game_id: i32, edit: GamesEditSpecific, db: &DatabaseConnection) -> Result<InsertReturn, DbErr> {
        ...
        match edit {
            GamesEditSpecific::RebuiltGame(edit) => {
                ...
                let game_data_active = ActiveModel {
                    id: Set(game_id),
                    fuel_shoot_teleop: Set(total_score.fuel_shoot_teleop),
                    fuel_pass_teleop: Set(total_score.fuel_pass_teleop),
                    fuel_shoot_auto: Set(total_score.fuel_shoot_auto),
                    fuel_pass_auto: Set(total_score.fuel_pass_auto),
                    climb_end: Set(total_score.climb_end),
                    climb_auto: Set(total_score.climb_auto),
                    beach_on_bump: Set(total_score.beach_on_bump),
                };
                game_data_active.update(db).await?;
                Ok(returne)
            },
        }
    }
